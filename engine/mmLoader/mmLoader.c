...
HMEMMODULE
LoadFileModule(_In_ LPCSTR lpFilename, _In_ BOOL bCallEntry, _Inout_ DWORD *pdwError) {
  PAPIPTR_TABLE pApis = InitApiTable();
  if (!pApis) {
    if (pdwError)
      *pdwError = MMEC_INVALID_WIN32_ENV;
  ...
  DWORD dwFileSize;
  	...
  	hFile = CreateFileA( lpFilename, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0 );
	...
	LPVOID lpPeModuleBuffer = MapViewOfFile( hMap, FILE_MAP_READ, 0, 0, 0 );
	if (lpPeModuleBuffer  == NULL) {
		CloseHandle( hMap );
		CloseHandle( hFile );
		return NULL;
	}
  ...
  if (LoadMemModuleInternal(pMemModule, lpPeModuleBuffer, bCallEntry)) {
    ...
    return (HMEMMODULE)pMemModule;
  	...
  	UnmapViewOfFile( lpPeModuleBuffer );
	CloseHandle( hMap );
	CloseHandle( hFile );
