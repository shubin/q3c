...
struct ft_wsi_info SDL_wsiInfo;
...
void GetWindowSize(const void *data, uint32_t *width, uint32_t *height) 
{
	int w, h;
...
void GetFramebufferSize(const void *data, uint32_t *width, uint32_t *height)
{
	int w, h;
	// TODO: other apis?
...
void GetVulkanInstanceExtensions(const void *data, uint32_t *count, const char **names) 
{
	unsigned int c;
	SDL_Vulkan_GetInstanceExtensions(SDL_window, &c, names);
	*count = c;
}

void CreateVulkanSurface(const void* data, VkInstance instance, const struct VkAllocationCallbacks* callbacks, VkSurfaceKHR* surface)
{
	// TODO: sdl doesn't provide the ability to use custom allocator
	SDL_Vulkan_CreateSurface(SDL_window, instance, surface);
}
...
{
	memset(&SDL_wsiInfo, 0, sizeof(SDL_wsiInfo));
...
success:
	...
	SDL_wsiInfo.get_vulkan_instance_extensions = GetVulkanInstanceExtensions;
	SDL_wsiInfo.create_vulkan_surface = CreateVulkanSurface;
	SDL_wsiInfo.get_framebuffer_size = GetFramebufferSize;
	SDL_wsiInfo.get_window_size = GetWindowSize;
}
