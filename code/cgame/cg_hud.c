...
static void hud_drawobituary( void ) {
	...
	for ( p = hud_obituary; p < hud_obituary + hud_numobituary; p++ ) {
		...
		if ( p->killer == p->victim || p->killer == ENTITYNUM_WORLD ) {
			hud_drawcolorstring( bounds.left + x, bounds.top + y, 0.4f, &font_regular, va( "%s ^7%s", cgs.clientinfo[p->victim].name, "suicides" ), shadow, 2, 2, qfalse );
			continue;
		}
		deathicon = media.icon_death;
		for ( i = 0; i < NUM_STAT_WEAPONS; i++ ) {
			if ( hud_stat_weapons[i] == p->weapon ) {
				deathicon = media.icon_stat_weapon[i];
				wepcolor[0] = hud_stat_weapon_colors[i][0];
				wepcolor[1] = hud_stat_weapon_colors[i][1];
				wepcolor[2] = hud_stat_weapon_colors[i][2];
				wepcolor[3] = color[3];
				break;
			}
		}
		dim = hud_measurecolorstring( 0.4f, &font_regular, cgs.clientinfo[p->killer].name );
		hud_drawcolorstring( bounds.left + x, bounds.top + y, 0.4f, &font_regular, cgs.clientinfo[p->killer].name, shadow, 2, 2, qfalse );
		hud_drawcolorstring( bounds.left + x + dim + 64, bounds.top + y, 0.4f, &font_regular, cgs.clientinfo[p->victim].name, shadow, 2, 2, qfalse );
		if ( media.icon_death != deathicon ) {
			trap_R_SetColor( wepcolor );
		}
		hud_drawpic( bounds.left + x + dim + 32, bounds.top + y - 8, 48, 48, 0.5f, 0.5f, deathicon );
		y += 40;
	}
	trap_R_SetColor( NULL );
}
